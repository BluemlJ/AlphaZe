...
CrazyAra::CrazyAra() : rawAgent(nullptr),
                       mctsAgent(nullptr),
...
void CrazyAra::uci_loop(int argc, char *argv[])
{
    ...
    do
    {
        ...
        else if (token == "uci")
        {
            ...
            set_uci_option(is, *state.get());
        else if (token == "go")
            go(state.get(), is, evalInfo);
        ...
        // Additional custom non-UCI commands, mainly for debugging
            ...
            export_search_tree(is);
        else if (token == "flip")
            state->flip();
        else if (token == "d")
            cout << *(state.get()) << endl;
        else if (token == "activeuci")
            activeuci();
#ifdef USE_RL
        ...
        // Test if the new modes are also usable for chess and others
            ...
            multimodel_arena(is, "", "", true);

        else if (token == "tournament")
            roundrobin(is);
#endif
        else
...
void CrazyAra::go(StateObj *state, istringstream &is, EvalInfo &evalInfo)
{
    ...
    while (is >> token)
    {
        if (token == "searchmoves")
            ...
            is >> searchLimits.time[WHITE];
        else if (token == "btime")
            is >> searchLimits.time[BLACK];
        else if (token == "winc")
            is >> searchLimits.inc[WHITE];
        else if (token == "binc")
            is >> searchLimits.inc[BLACK];
        else if (token == "movestogo")
            is >> searchLimits.movestogo;
        else if (token == "depth")
            is >> searchLimits.depth;
        else if (token == "nodes")
            is >> searchLimits.nodes;
